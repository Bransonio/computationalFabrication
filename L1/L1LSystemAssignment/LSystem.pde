...
class LSystem extends BaseLSystem {
 ...
 // Production rules
  ...
  public void iterate() {
    ...
    // [SOLUTION]: I loop through all characters in current. Anytime I encounter a character with a rule, 
    // the rule's output is appended to the buffer. If the indexed character has no rule, it is treated as 
    // constant, and appended to the buffer unchanged.

    // Iterate through all characters in current iteration string.
    for (int i = 0; i < current.length(); i++) {
      char currentChar = current.charAt(i);
      if (rules.get(currentChar) != null) {
        // If rules are found for this character, implement them in buffer...
        currentIterationBuffer.append(rules.get(currentChar));
      }
      else {
        // ... and if not, treat the character as a constant.
        currentIterationBuffer.append(currentChar);
      }
    }
